...
public plugin_init()
{
	...
	register_clcmd("rsh_reload_file", "Command_Reload_RanksFile", ADMIN_RCON, "- Reload settings and rank file..")
...
public plugin_cfg()
{
	...
	static szRed[6], szGreen[6], szBlue[6]

	parse(g_eSettings[HudColors], szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue))

	g_iHudSettings[Red] = str_to_num(szRed)
	g_iHudSettings[Green] = str_to_num(szGreen)
	g_iHudSettings[Blue] = str_to_num(szBlue)
	g_iHudSettings[X_Coord] = g_eSettings[HudX]
	g_iHudSettings[Y_Coord] = g_eSettings[HudY]
	g_iHudSettings[Effect] = g_eSettings[HudEffect]
}
...
public Read_Ranks_File(bool:bReloadFile)
{
					...
					switch(iSection)
					{
						case SECTION_SETTINGS:
						{
							...
							else if (equal(szKey, "HUD_EFFECT"))
							{
								...
								if (g_eSettings[HudEffect] > 1)
									g_eSettings[HudEffect] = 0
							}
...
UTIL_FormatHudMessage(id, iDeadId)
{
	static szRankName[eRankNameHud][64], szHudMessage[MAX_FMT_LENGTH], iNextLevelExp

	ArrayGetString(g_aRankName, g_iPlayerData[iDeadId][Level], szRankName[CURRENT_RANK], charsmax(szRankName[]))			

	if (g_iPlayerData[iDeadId][Level] > iTotalRanks - 1)
	{
		formatex(szHudMessage, charsmax(szHudMessage), "Rank: %s^nLevel: %i^nExperience: %i", 
			szRankName[CURRENT_RANK], g_iPlayerData[iDeadId][Level], g_iPlayerData[iDeadId][Experience])
	}
	else
	{
		iNextLevelExp = ArrayGetCell(g_aRankExp, g_iPlayerData[iDeadId][NextLevel])
		ArrayGetString(g_aRankName, g_iPlayerData[iDeadId][NextLevel], szRankName[NEXT_RANK], charsmax(szRankName[]))	

		formatex(szHudMessage, charsmax(szHudMessage), "Rank: %s^nLevel: %i/%i^nExperience: %i/%i^nNext Rank: %s", 
			szRankName[CURRENT_RANK], g_iPlayerData[iDeadId][Level], iTotalRanks - 1, g_iPlayerData[iDeadId][Experience], iNextLevelExp, szRankName[NEXT_RANK])
	}

	set_hudmessage(g_iHudSettings[Red], g_iHudSettings[Green], g_iHudSettings[Blue], g_iHudSettings[X_Coord], g_iHudSettings[Y_Coord], g_iHudSettings[Effect], 0.95, 0.95)
	ShowSyncHudMsg(id, g_SyncHudMessage, szHudMessage)
}
